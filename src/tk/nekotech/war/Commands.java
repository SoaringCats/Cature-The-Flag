	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (label.equals("join")) {
			if (sender instanceof Player) {
				...
				if (!war.teamhelpers.onTeam(p)) {
					if (war.blu.size() == war.red.size()) {
						Random rand = new Random();
						int decider = rand.nextInt(2);
						war.assignment.assignPlayer(p, decider);
					} else if (war.blu.size() < war.red.size()) {
						war.assignment.assignPlayer(p, 0);
					} else if (war.red.size() < war.blu.size()) {
						war.assignment.assignPlayer(p, 1);
					}
				} else {
					p.sendMessage(ChatColor.DARK_RED + "You're already on a team, dolt!");
					String team = null;
					if (war.teamhelpers.teamName(p) == 0) {
						team = "blu";
					} else if (war.teamhelpers.teamName(p) == 1) {
						team = "red";
					} else {
